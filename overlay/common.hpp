...
// kept separate on purpose because it is a shared struct in memory that may change
...
inline bool pointInRect(const POINTS& pt, WindowRect& rc)
{
    return pt.x >= rc.x
        && pt.y >= rc.y
        && pt.x <= rc.x + rc.width
...
inline bool pointInRect(const POINT& pt, WindowRect& rc)
{
    return pt.x >= rc.x
        && pt.y >= rc.y
        && pt.x <= rc.x + rc.width
...
inline int hitTest(POINT pt, WindowRect& rect, bool resizable, WindowCaptionMargin& caption, int borderWidth)
{
