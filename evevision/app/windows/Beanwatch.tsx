...
class Beanwatch extends Component<BeanwatchProps, BeanwatchState> {
    ...
    click = () => {
        ...
        if(!this.state.panic && !this.state.cancelling) { // normal, not panicking or cancelling
            if(newClicks >= 3) {
                ...
                this.setState({...this.state, buttonClicks: 0, panic: true, buttonHovered: false})
                this.sendPanicAlert();
            } else {
                this.setState({...this.state, buttonClicks: newClicks})
            }
        } else if(this.state.panic){ // currently panicking
            if(newClicks >= 2) {
                this.setState({...this.state, buttonClicks: 0, cancelling: true, panic: false, panicConfirmed: false})
    ...
    buttonText(): String {
        if(this.state.panic && !this.state.panicConfirmed) {
            ...
            switch(this.state.buttonClicks) {
                case 0: return this.state.buttonHovered ? 'CANCEL?' : 'FLEET EN ROUTE';
                case 1: return 'CONFIRM CANCEL';
            ...
            switch(this.state.buttonClicks) {
                ...
                case 2: return 'SOUND THE HORDE HORN?';
                default: return 'YOU BROKE SOMETHING!';
            }
        }
    }
    ...
    render() {
        if(this.props.auth === undefined) {
            ...
            return (
                <>
                    <div className={this.buttonClass()} onMouseLeave={this.mouseLeave} onMouseEnter={this.mouseEnter} onClick={this.click}>{this.buttonText()}</div>
                    <Panel>
                        <Typography>Beanwatch development is almost complete! Working out the politics and the ansiblex routing. Should be in the next release!</Typography>
                    </Panel>
                </>
            )
        }
