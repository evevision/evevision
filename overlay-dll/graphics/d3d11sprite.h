...
inline void D3d11SpriteDrawer::init(HMODULE hModuleD3dCompiler47, int swapChainWidth, int swapChainHeight, bool gammaFix)
{
    ...
    if (fn)
    {
        ...
        if (FAILED(hr))
        {
            __trace__ << L"compile PShader hr:" << hr;
            if (errorBlob) {
                error = std::string((char*)errorBlob->GetBufferPointer());
                std::cout << "compile PShader error:" << error << std::endl;
                __trace__ << L"compile PShader:" << error;
            }
            return;
        }
        ...
        if (FAILED(hr))
        {
            __trace__ << L"CreateVertexShader hr:" << hr;
            return;
        }
        ...
        if (FAILED(hr))
        {
            __trace__ << L"CreatePixelShader hr:" << hr;
            return;
        }
        ...
        if (FAILED(hr))
        {
            __trace__ << L"CreateInputLayout hr:" << hr;
            return;
        }
    }
}
...
inline void D3d11SpriteDrawer::drawUnscaleSprite(Windows::ComPtr<ID3D11Texture2D> texture, RECT& rc, D3DCOLOR color)
{
    ...
    if (FAILED(hr))
    {
        __trace__ << L"drawUnscaleSprite CreateShaderResourceView failed:" << hr;
        ...
        if (hr == DXGI_ERROR_DEVICE_REMOVED)
        {
            ...
            __trace__ << L"drawUnscaleSprite CreateShaderResourceView failed GetDeviceRemovedReason:" << hr;
        }
        return;
    }
    ...
    else
    {
        ...
        __trace__ << "drawUnscaleSprite CreateBuffer failed :" << hr;
        if (hr == DXGI_ERROR_DEVICE_REMOVED)
        {
            hr = m_d3dDevice->GetDeviceRemovedReason();
            std::cout << "GetDeviceRemovedReason :" << hr << std::endl;
...
inline void D3d11SpriteDrawer::drawScaleSprite(Windows::ComPtr<ID3D11Texture2D> texture, RECT& srcRect, RECT& dstRect, D3DCOLOR color /*= 0xffffffff*/)
{
    ...
    if (FAILED(hr))
    {
        ...
        __trace__ << "drawScaleSprite CreateShaderResourceView failed :" << hr;

        if (hr == DXGI_ERROR_DEVICE_REMOVED)
        {
            hr = m_d3dDevice->GetDeviceRemovedReason();
            std::cout << "GetDeviceRemovedReason :" << hr << std::endl;
            __trace__ << "drawScaleSprite GetDeviceRemovedReason :" << hr;
        }
        return;
    }
    ...
    {
        __trace__ << "drawScaleSprite CreateBuffer failed:" << hr;
