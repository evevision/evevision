...
export default class ApiClient {
    ...
    if (character !== undefined && character.auth !== undefined) {
      ...
      if (priorAuth === undefined) {
        ...
        if (this.auth.expiresAt - new Date().getTime() < 5 * 60 * 1000) {
          ...
          console.log(
            "WSAPI-" + this.characterId + ": refreshing token before connection"
          );
        ...
        if (priorAuth.accessToken !== this.auth.accessToken) {
          ...
          if (this.refreshingBeforeConnect) {
            ...
            console.log(
              "WSAPI-" + this.characterId + ": connecting after token refresh"
            );
            this.connect();
          }
  ...
  private refreshAccessToken = async () => {
    if (this.auth !== undefined) {
      ...
      try {
        ...
        console.log(
          "WSAPI-" + this.characterId + ": successfully refreshed access token"
        );
        store.dispatch(
          updateCharacterAuth(
        ...
        console.error(
          "WSAPI-" + this.characterId + ": Error getting oauth token",
          error
        );
      }
    }
  ...
  private clearRefreshTimer() {
    if (this.refreshTimer !== undefined) {
      console.log(
        "WSAPI-" + this.characterId + ": clearing existing refresh timer"
      );
  ...
  private startRefreshTimer() {
    if (this.auth !== undefined) {
      ...
      const refreshMs =
        this.auth.expiresAt - new Date().getTime() - 5 * 60 * 1000;
      console.log(
        "WSAPI-" +
          this.characterId +
          ": refresh timer set for " +
          (refreshMs / 1000 / 60).toFixed(1) +
          " minutes"
      );
  ...
  private handleClose = () => {
    if (this.connecting) {
      ...
      console.log(
        "WSAPI-" + this.characterId + ": connection successfully closed"
      );
    }
  };
  ...
  private handleError = (err: Error) => {
    if (this.connecting) {
      ...
      console.error(
        "WSAPI-" + this.characterId + ": CONNECTION ERROR",
        err.message
      );
    } else {
  ...
  private handleMessage = (_message: object) => {
    // hi
  };
  ...
  private connect() {
    ...
    this.socket = new WebSocket(
      "wss://api.eve.vision/?token=" + encodeURI(this.auth.accessToken)
    )
