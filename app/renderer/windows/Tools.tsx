          ...
          <Button
            onClick={() => {
              ipcRenderer.send(
                "openWindow",
                "externalsite",
                "https://zkillboard.com/character/" + this.props.character!.id
              );
            }}
          >
            ZKillboard
...
const mapStateToProps = (state: AppState, ownProps: ToolsProps) => {
  const character = state.characters.characters.find(
    (c: { id: number }) => c.id === ownProps.characterId
  );
