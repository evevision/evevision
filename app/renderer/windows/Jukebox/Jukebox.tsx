...
import { faBackward, faForward, faPlay, faPause, faRandom } from '@fortawesome/free-solid-svg-icons'
...
class Jukebox extends React.PureComponent<{}, JukeboxState> {
    ...
    updateTrack = (track: {title: string, number: number, duration: string, soundCloudTrackId: number}) => {
    ...
    nextTrack = () => {
        ...
        if (this.state.shuffle) {
            ...
            const currentTrackIndex = playlist.tracks.findIndex(track => track.soundCloudTrackId == this.state.currentTrack?.soundCloudTrackId);
    ...
    prevTrack = () => {
        ...
        if (this.state.shuffle) {
          ...
          const currentTrackIndex = playlist.tracks.findIndex(track => track.soundCloudTrackId == this.state.currentTrack?.soundCloudTrackId);
          prevTrackIndex = (currentTrackIndex - 1 + playlist.tracks.length) % playlist.tracks.length;
        }
        const nextTrack = playlist.tracks[prevTrackIndex];
        return this.setState({...this.state, currentTrack: nextTrack, paused: false});
    }
    ...
    render() {
        return (
            <>
                ...
                <Panel>
                    <TextInput value={this.state.currentTrack ? `${millisToMinutesAndSeconds(this.state.positionMilliseconds)} \t ${this.state.currentTrack.title}` : ""} readOnly style={{width: "100%"}}></TextInput>
                    <div className={styles["eve-jukebox-button-list"]}>
                        <div>
                            ...
                            <Button onClick={this.shuffle}>
                                <FontAwesomeIcon icon={faRandom} color={this.state.shuffle ? "#9df799": undefined} fixedWidth />
                            </Button>
                        </div>
                        <div>
                            <SliderInput min={0} max={100} value={this.state.volume} style={{width: "75px"}} onChange={this.updateVolume}/>
                        </div>
                    </div>
                    <div className={`${styles["eve-jukebox-playlist-container"]} eve-scrollbar`}>
                        <JukeboxPlaylist playList={playlist} currentTrack={this.state.currentTrack} onTrackClick={this.updateTrack} />
                    </div>
                </Panel>
            </>
        );
